...
class Render extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('backend/Render_model', 'render_model');
	...
	public function pdf()
	{
		...
		$post = $this->render_model->get_edit_data($id_kunjungan);
		...
		$result_dokter = $this->render_model->get_karyawan_by_nama($dokters);
	...
	$invoice_data = $this->render_model->get_invoice_data($id_kunjungan);
	$lab_rows = '';
	$last_kunjungan = null;
	...
	<tr><td width="20%" style="border:1px solid black;"><b>Penunjang Lain</b></td><td width="80%" style="border:1px solid black;">' . htmlspecialchars($decodedData['penunjang_lain']) . '</td></tr>
		...
		$resep_list = $this->render_model->get_resep_by_kunjungan($id_kunjungan);
		...
		foreach ($resep_list as $resep) {
		    $obat_rows .= '<tr>
		        ...
		        <td width="7%">' . htmlspecialchars($resep['dosis']) . '</td>
		        <td width="10%">' . htmlspecialchars($resep['frekuensi']) . '</td>
		        <td width="33%">' . htmlspecialchars($resep['eticket_a']) . ' ' . htmlspecialchars($resep['eticket_b']) . ' ' . htmlspecialchars($resep['eticket_c']) . '</td>
		    </tr>';
		}
		...
		// OUTPUT
		ob_end_clean();
		$pdf->Output($namaRM . '-' . $judulRM . '.pdf', 'I');
	}
