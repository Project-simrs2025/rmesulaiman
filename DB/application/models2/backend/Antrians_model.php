...
class Antrians_model extends CI_Model
{
	...
	{
	    $this->postgres->select('antrian.*, antrian.waktu_masuk, patient.nama AS patient_name, patient.no_rm AS patient_record, penjamin.nama AS penjamin_name,k.nama as dokter_name,poly.name AS poly_name,antrian.id_poly AS poli_id, antrian.id AS id_antrian, patient.id AS patient_id');
	    ...
	    $this->postgres->where('antrian.id_poly ', '2');
	    $this->postgres->order_by('poly.name', 'asc'); 
		$this->postgres->order_by('antrian.waktu_masuk', 'asc');  
	...
	{
		$this->postgres->select('antrian.*, patient.nama AS patient_name, patient.no_rm AS patient_record, penjamin.nama AS penjamin_name,k.nama as dokter_name,poly.name AS poly_name,antrian.id_poly AS poli_id, antrian.id AS id_antrian, patient.id AS patient_id');
		$this->postgres->join('master_pasien patient', 'patient.id = antrian.id_pasien', 'left');
		$this->postgres->join('master_kategori_harga penjamin', 'CAST(penjamin.id AS varchar) = CAST(antrian.id_kategori_harga AS varchar)', 'left');
    ...
    public function pulangkan($idpas, $kunjungan, $antrian)
	{
	    // Data khusus untuk tabel antrian
	    $data_antrian = [
	        'status_antrian' => 'N',
	        'status_aktif' => 'N',
	    ...
	    // Data khusus untuk tabel master_pasien
	    $data_pasien = [
	        'status_kunjungan' => 'N'
	    ];

	    // Data khusus untuk tabel kunjungan
	    $data_kunjungan = [
	        'status_kunjungan' => 'N'
	    ];

	    // Data khusus untuk invoice (misalnya juga status_issue)
	    $data_invoice = [
	        'status_issue' => 1
	    ];

	    $update_antrian = $this->postgres->update('antrian', $data_antrian, ['id' => $antrian]);
	    $update_pasien = $this->postgres->update('master_pasien', $data_pasien, ['id' => $idpas]);
	    $update_kunjungan = $this->postgres->update('kunjungan', $data_kunjungan, ['id' => $kunjungan]);
	    $update_invoice = $this->postgres->update('transaksi_invoice', $data_invoice, [
	        'id_pasien' => $idpas,
	        'id_kunjungan' => $kunjungan
	    ]);

	    return $update_antrian && $update_pasien && $update_kunjungan && $update_invoice;
	}
	...
	// {
	// 	$this->postgres->select('admission.*, p.nama AS nama_pasien, lt.nama_lantai, ru.nama_ruangan, k.nama AS nama_jenis_pasien,bed.no_bad,dokter.nama as nama_dokter,poli.name as nama_poli, p.nik, p.tanggal_lahir, pk.nama as nama_pekerjaan, jk.nama as jenkel, p.alamat, agama.nama as nama_agama');
