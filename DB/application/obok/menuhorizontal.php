	...
	<script>
		class MarkerManager {
			constructor(selector) {
				...
				this.prevImages = new Map(); // Store states per image
			...
			initialize() {
				this.images.forEach(img => {
					...
					this.prevImages.set(inputName, img.src);
					const currentState = document.querySelector(`#${inputName}`).value;
					if (currentState !== "null" && currentState !== "") {
						const state = JSON.parse(currentState)
						this.prevStates.set(inputName, state);
			...
			showMarkerArea(target) {
				...
				// Save the marker state when closed
				markerArea.addEventListener("close", () => {
					console.log('close');

					const id = target.getAttribute("data-input-name");
					document.getElementById(id).value = JSON.stringify(this.prevStates.get(inputName) || {});
				});
				...
				if (this.prevStates.has(inputName)) {
					console.log("Restoring state for:", inputName, this.prevStates.get(inputName));
					markerArea.restoreState(this.prevStates.get(inputName));
				}

				// Apply Bootstrap modal styles
				// if (markerModal && this.prevStates.has(inputName)) {
				//     markerModal.classList.add('modal', 'in', 'fade');
				//     markerModal.setAttribute('role', 'dialog');
				//     markerModal.setAttribute('aria-labelledby', 'markerModalLabel');
		...
		<div class="layout-horizontal">
			<header class="mb-5">
				...
				<nav class="main-navbar">
					<div class="container">
						<ul>
							<?php if ($this->session->userdata('level') == '13' || $this->session->userdata('level') == '5'): ?>
							...
							<?php else: ?>
								<li class="menu-item">
									<a href="#" class='menu-link'>
										<i class="bi bi-grid-1x2-fill"></i>
										<span> Anda Penyusup, Tidak ada Menu Untuk Anda!! ;)</span>
									</a>
								</li>
							<?php endif; ?>
