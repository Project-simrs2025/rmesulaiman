...
<script>
	...
	function calculateScoreNutrition() {
		const radioNames = ['tampak_kurus', 'penurunan_bb', 'kondisi_khusus', 'risiko_malnutrisi'];
		let totalScore = 0;
		...
		// Masukkan ke input skor
		const inputScore = document.getElementById('score_nutrisi');
		if (inputScore) {
			inputScore.value = totalScore;
		}

		// Tampilkan atau sembunyikan keterangan
		const keteranganEl = document.getElementById('keterangan_nutrisi');
		if (keteranganEl) {
			if (totalScore >= 1) {
				keteranganEl.textContent = 'Kolaborasi dengan ahli gizi diperlukan.';
			} else {
	...
	function calculateScoreParameter() {
		const radioNames = [
			...
			'parameter_respon_pembedahan',
			'parameter_penggunaan_obat'
		];
		let totalScore = 0;
		...
		const scoreInput = document.getElementById('score_parameter');
		if (scoreInput) {
