...
class Admission extends CI_Controller
{
	...
	public function index()
	{
		...
		$getdataruang = $this->admission_model->get_list_ruang();
		$lt = array('' => '[Pilih ruang]');
		foreach ($getdataruang as $x) {
			$lt[$x] = $x;
		}
		$data['form_filter'] = form_dropdown('', $opt, '', 'id="dokter" class="form-select dokter" style="width:100%;"');
		$data['form_filter_ruang'] = form_dropdown('', $lt, '', 'id="ruang" class="form-select ruang" style="width:100%;"');
	...
	function getradio()
	{
		...
		$data = $this->admission_model->get_all_radio($searchQuery, $limit, $offset);
		$totalCount = $this->admission_model->count_all_radio($searchQuery);
