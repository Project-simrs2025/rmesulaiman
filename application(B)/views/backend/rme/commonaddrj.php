			...
			<div class="row">
				<div class="col-12 col-md-12 mb-2 float-end">
					<div class="d-flex justify-content-end gap-4 flex-row" id="footer">
						...
						<button type="button" class="btn btn-light kembali" id="btnBatal">
...
<script type="application/javascript">
	...
	function onSubmit(event, mode = 'add') {
	...
	function addAPI(data) {
		...
		$.ajax({
			...
			success: (data) => {
				...
				if (!data.status) {
				    if (data.message) {
				        // Tampilkan SweetAlert error popup
				        Swal.fire({
				            icon: 'error',
				            title: 'Oops...',
				            text: data.message,
				...
				const rm = $('#no_rm').val();
				$("#data_pasien").load('content/digital/ass_resiko_jatuh/data.php?id=' + rm);

				// redirect ke halaman Web A setelah 1 detik (atau hapus jika tidak mau redirect)
				setTimeout(() => {
					window.location.href = 'http://192.168.55.99/pmo/pasien-detail-' + rm;
				}, 1000);
	...
	$(document).on('click', '#btnBatal', function(e) {
	    ...
	    if (location.hostname === '192.168.55.99' && location.pathname.includes('/pmo')) {
	        // Jika ini berada di halaman simrs-pamela, load konten
	        $('#data_pasien').load('/content/digital/ass_resiko_jatuh/data.php?id=' + rm);
	    } else {
	        // Jika bukan di simrs-pamela, redirect ke detail pasien
	        window.location.href = 'http://192.168.55.99/pmo/pasien-detail-' + rm;
	    }
	});
