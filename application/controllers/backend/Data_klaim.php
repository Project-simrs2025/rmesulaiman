...
class Data_klaim extends CI_Controller
{
	...
	public function _render_pdfrm9($id_kunjungan, $pdf)
	{
		...
		if ($get_id_konten->num_rows() > 0) {
			...
			if ($admission_id_kunjungan == NULL) {
				...
				$tgl_admit2 = date('Y-m-d', strtotime($post->tgl_admit)); 
    ...
    : '-';
		...
		// Awal HTML
		// Cek jika path valid dan file ada
		$html1 .= '<img src="' . $path_ttd_url_perawatTriase . '" width="80" height="80" /><br>
    ...
    : '-';
		...
		// Awal HTML
		// Cek jika path valid dan file ada
		$html1 .= '<img src="' . $path_ttd_url_rmCurratage . '" width="80" height="80" /><br>
    ...
    : '-';
					...
					<tr>
						<td colspan="3">
							...
							<table border="1" cellpadding="4">
								<tr>
									<td>
										' . $decodedData['penilaian_pra_induksi'] . ' <br><br>
									</td>
								</tr>
							</table>
						</td>
		...
		// Awal HTML
		// Cek jika path valid dan file ada
		$html1 .= '<br><div align="right"><img src="' . $path_ttd_url_rmLapAnestesi . '" width="80" height="80"/></div>
    ...
    : '-';
		...
		<img src="' . $path_ttd_url_DokterUmumrm18e . '" width="80" height="80" /><br>
		' . $decodedData['dokter_umum'] . '
		</div>
		';
...
';
		...
		// Awal HTML
		// Cek jika path valid dan file ada
		$html1 .= '<img src="' . $path_ttd_url_rmDokterPenataAnestesi . '" width="80" height="80" /><br>
		...
		// Awal HTML
		// Cek jika path valid dan file ada
		$html1 .= '<img src="' . $path_ttd_url_rmPenataAnestesi . '" width="80" height="80" /><br>
    ...
    : '-';
		...
		for ($i = 1; $i <= 30; $i++) {
		    ...
		    // === QR Perawat BENAR ===
			$safe_perawat_1e = $this->to_safe_filename($perawat) . '.png';
			$path_ttd_server_1e = 'C:/xampp/htdocs/pmo/images/qrcodeIT/' . $safe_perawat_1e;
			$path_ttd_url_1e = BASE_STORAGE . '/pmo/images/qrcodeIT/' . $safe_perawat_1e;
		    $qr_html_perawat = file_exists($path_ttd_server_1e)
		        ? '<img src="' . $path_ttd_url_1e . '" width="60" height="60" /><br/>' . $perawat
		        : $perawat;

		    // === QR Dokter ===
			$safe_dokter_1e = $this->to_safe_filename($dokter) . '.png';
			$path_ttd_server_dokter_1e = 'C:/xampp/htdocs/pmo/images/qrcodeIT/' . $safe_dokter_1e;
			$path_ttd_url_dokter_1e = BASE_STORAGE . '/pmo/images/qrcodeIT/' . $safe_dokter_1e;
		    $qr_html_dokter = file_exists($path_ttd_server_dokter_1e)
		        ? '<img src="' . $path_ttd_url_dokter_1e . '" width="60" height="60" /><br/>' . $dokter
		        : $dokter;
    ...
    : '-';
		...
		for ($i = 0; $i < count($nama_ruanganPasien); $i++) {
			...
			if (file_exists($path_ttd_server_anestesi_rm3a)) {
				$html1 .= '		
					<img src="' . $path_ttd_url_anestesi_rm3a . '" width="80" height="80" /><br>
				';
			}
			...
			if (file_exists($path_ttd_server_penerima_anestesi_rm3a)) {
				$html1 .= '		
					<img src="' . $path_ttd_url_penerima_anestesi_rm3a . '" width="80" height="80" /><br>
				';
			}
	...
	public function _render_pdfrm28($id_kunjungan, $pdf)
	{
		...
		$getsettings = $this->site_model->get_settings_data();
		$site_title = $getsettings['nama'];
		$imagesrm28 = BASE_STORAGE . '/pmo/images/' . $getsettings['logo'];

		// Ambil data FORM RME
		$berkas_klaim = 'rm28_new';
		$formData = $this->data_klaim_model->get_by_id($id_kunjungan, $berkas_klaim);
		if (!$formData) return;
		...
		// HEADER PDF
		// // Matikan GAMBAR HEADER
		// $pdf->Image($imagesrm28, 10, 10, 20, 20, '', '', 1, 0);
		// $pdf->SetFont('times', 'B', 16);
		// $pdf->Cell(190, 4, $site_title, 0, 1, 'C');
		// $pdf->SetFont('times', 'B', 10);
		// $pdf->MultiCell(45, 5, '', 0, 'C', 0, 0, '', '', true);
		...
		function checkboxrm28($value, $label = '')
		{
			return (!empty($value) && $value !== '0') ? '[v] ' . $label : '[ ] ' . $label;
		}

		function radiomarkrm28($value, $expectedValue, $label)
		{
			return ($value === $expectedValue) ? '[v] ' . $label : '[ ] ' . $label;
		}
				...
				<tr>
					<td width="55%">
						<table>
							<tr>
								<td width="13%">
									<img src="' . $imagesrm28 . '" width="130" height="130"/>
								</td>
								<td width="87%">
									' . $site_title  . '<br>
									' . $lokasi . ' <br>
		...
		$html1 .= '<table border="1" cellpadding="5" cellspacing="0" width="100%">
			<tr style="font-weight: bold;">
				<td colspan="2" width="80">Ruang Rawat Awal</td>
				<td width="619">' . $decodedData['ruang_rawat_awal'] . '</td>
				<td width="2768">
					Riwayat Alergi : <br>
					' . checkboxrm28($decodedData['tidak_alergi']) . ' Tidak <br>
					' . checkboxrm28($decodedData['ya_alergi']) . 'Ya, sebutkan : &nbsp; 
					' . $decodedData['sebutkan_alergi_apa'] . '
				</td>
			</tr>
			<tr>
				<td rowspan="3" width="20">No</td>
				<td width="60">Order</td>
				<td rowspan="3" width="60">Nama Obat</td>
				<td rowspan="3" width="50">Dosis</td>
				<td colspan="5" width="150">Frekuensi Pemberian</td>
				<td rowspan="3" width="50">Rute</td>
				<td rowspan="3" width="60">Informasi/ Keterangan</td>
				<td colspan="2" width="86">Durasi Terapi</td>
				<td rowspan="3" width="80">Nama Dokter</td>
				<td rowspan="3" width="43">Review Farmasi</td>
				<td rowspan="3" width="40">Hari</td>';

		for ($i = 1; $i <= 16; $i++) {
			$tanggalKey = 'tanggal_farmasi_' . $i;
			$tanggalValue = isset($decodedData[$tanggalKey]) ? $decodedData[$tanggalKey] : '';
			$html1 .= '<td width="173">Tanggal &nbsp; ' . $tanggalValue . '</td>';
		}
		...
		$html1 .= '
			...
			<tr>
				...
				<td rowspan="2" width="30">Tiap 24 Jam</td>
				<td rowspan="2" width="30">Tiap 12 Jam</td>
				<td rowspan="2" width="30">Tiap 8 Jam</td>
				<td rowspan="2" width="30">Tiap 6 Jam</td>
				<td rowspan="2" width="30">Tiap ... Jam</td>
				<td width="43">Mulai</td>
				<td width="43">Stop</td>';
		...
		$tanggal_jam = $decodedData['tanggal_jam_order'] ?? [];
		$nama_obat = $decodedData['nama_obat'] ?? [];
		$dosis_obat = $decodedData['dosis_obat'] ?? [];
		$jamcheck24jam = $decodedData['perawatPertama'] ?? [];
		$jamcheck12jam = $decodedData['perawatKedua'] ?? [];
		$jamcheck8jam = $decodedData['perawatKetiga'] ?? [];
		$jamcheck6jam = $decodedData['perawatKeempat'] ?? [];
		$jamcheck3jam = $decodedData['perawatKelima'] ?? [];
		$cara_obat = $decodedData['cara_obat'] ?? [];
		$keterangan = $decodedData['ket_obat'] ?? [];
		$durasiPertama = $decodedData['tanggal_jam'] ?? [];
		$durasiKedua = $decodedData['tanggal_jam_kedua'] ?? [];
		$dokter_nama = $decodedData['perawat_nama'] ?? [];
		$farmasiCheck = $decodedData['reviewFarmasi'] ?? [];



		//alergi baris 1
		$RuangRawatan1 = $decodedData['ruang_rawatan'] ?? [];
		$KetjamRawatPertama1 = $decodedData['ket_jam_pertama'] ?? [];
		$KetjamRawatKedua1 = $decodedData['ket_jam_kedua'] ?? [];
		$KetjamRawatKetiga1 = $decodedData['ket_jam_ketiga'] ?? [];
		$KetjamRawatKeempat1 = $decodedData['ket_jam_keempat'] ?? [];
		//checked
		$PertamaCheck1 = $decodedData['checkjamKolom1Pertama'] ?? [];
		$KeduaCheck1 = $decodedData['checkjamKolom1Kedua'] ?? [];
		$KetigaCheck1 = $decodedData['checkjamKolom1Ketiga'] ?? [];
		$KeempatCheck1 = $decodedData['checkjamKolom1Keempat'] ?? [];
		//name perawat Q // suntik shift pagi tanggal 1
		$namaPerawatQPertama1 = $decodedData['perawat_nama_mengecek_pertama'] ?? [];
		$namaPerawatQKedua1 = $decodedData['perawat_nama_mengecek_xq_pertama'] ?? [];
		$namaPerawatQKetiga1 = $decodedData['perawat_nama_mengecek_xw_pertama'] ?? [];
		$namaPerawatQKeempat1 = $decodedData['perawat_nama_mengecek_xe_pertama'] ?? [];
		//name perawat W // suntik shift sore tanggal 1
		$namaPerawatWPertama1 = $decodedData['perawat_nama_mengecek_kedua'] ?? [];
		$namaPerawatWKedua1 = $decodedData['perawat_nama_mengecek_xq_kedua'] ?? [];
		$namaPerawatWKetiga1 = $decodedData['perawat_nama_mengecek_xw_kedua'] ?? [];
		$namaPerawatWKeempat1 = $decodedData['perawat_nama_mengecek_xe_kedua'] ?? [];
		//batas


		for ($i = 2; $i <= 16; $i++) {
			// Ruang rawatan
			${"RuangRawatan{$i}"} = $decodedData["ruang_rawatan_kolom{$i}"] ?? [];

			// Keterangan jam rawat
			${"KetjamRawatPertama{$i}"} = $decodedData["ket_jam_pertama_kolom{$i}"] ?? [];
			${"KetjamRawatKedua{$i}"} = $decodedData["ket_jam_kedua_kolom{$i}"] ?? [];
			${"KetjamRawatKetiga{$i}"} = $decodedData["ket_jam_ketiga_kolom{$i}"] ?? [];
			${"KetjamRawatKeempat{$i}"} = $decodedData["ket_jam_keempat_kolom{$i}"] ?? [];

			// Checked
			${"PertamaCheck{$i}"} = $decodedData["checkjamKolom{$i}Pertama"] ?? [];
			${"KeduaCheck{$i}"} = $decodedData["checkjamKolom{$i}Kedua"] ?? [];
			${"KetigaCheck{$i}"} = $decodedData["checkjamKolom{$i}Ketiga"] ?? [];
			${"KeempatCheck{$i}"} = $decodedData["checkjamKolom{$i}Keempat"] ?? [];

			// Nama Perawat Q (shift pagi)
			${"namaPerawatQPertama{$i}"} = $decodedData["perawat_nama_mengecek_pertama_{$i}"] ?? [];
			${"namaPerawatQKedua{$i}"} = $decodedData["perawat_nama_mengecek_xq_pertama_{$i}"] ?? [];
			${"namaPerawatQKetiga{$i}"} = $decodedData["perawat_nama_mengecek_xw_pertama_{$i}"] ?? [];
			${"namaPerawatQKeempat{$i}"} = $decodedData["perawat_nama_mengecek_xe_pertama_{$i}"] ?? [];

			// Nama Perawat W (shift sore)
			${"namaPerawatWPertama{$i}"} = $decodedData["perawat_nama_mengecek_kedua_{$i}"] ?? [];
			${"namaPerawatWKedua{$i}"} = $decodedData["perawat_nama_mengecek_xq_kedua_{$i}"] ?? [];
			${"namaPerawatWKetiga{$i}"} = $decodedData["perawat_nama_mengecek_xw_kedua_{$i}"] ?? [];
			${"namaPerawatWKeempat{$i}"} = $decodedData["perawat_nama_mengecek_xe_kedua_{$i}"] ?? [];
		}
		...
		for ($i = 0; $i < count($tanggal_jam); $i++) {
			if (empty($tanggal_jam[$i]) && empty($nama_obat[$i])) continue;

			$tgljam = !empty($tanggal_jam[$i]) ? date('d-m-Y H:i', strtotime($tanggal_jam[$i])) : '-';
			$nama_obat_val = $nama_obat[$i] ?? '-';
			$dosis = $dosis_obat[$i] ?? '-';
			$rute = $cara_obat[$i] ?? '-';
			$ket = $keterangan[$i] ?? '-';
			$durPertama = $durasiPertama[$i] ?? '-';
			$durKedua = $durasiKedua[$i] ?? '-';
			$dokter_obat = $dokter_nama[$i] ?? '-';

			//alergi baris 1
			$RuangPasien1 = $RuangRawatan1[$i] ?? '-';
			$KjamRawatPertama1 = $KetjamRawatPertama1[$i] ?? '-';
			$KjamRawatKedua1 = $KetjamRawatKedua1[$i] ?? '-';
			$KjamRawatKetiga1 = $KetjamRawatKetiga1[$i] ?? '-';
			$KjamRawatKeempat1 = $KetjamRawatKeempat1[$i] ?? '-';
			//checked
			$is_checkedPertama1 = $PertamaCheck1[$i] ?? '0';
			$checkPertama1 = checkboxrm28($is_checkedPertama1);
			$is_checkedKedua1 = $KeduaCheck1[$i] ?? '0';
			$checkKedua1 = checkboxrm28($is_checkedKedua1);
			$is_checkedKetiga1 = $KetigaCheck1[$i] ?? '0';
			$checkKetiga1 = checkboxrm28($is_checkedKetiga1);
			$is_checkedKeempat1 = $KeempatCheck1[$i] ?? '0';
			$checkKeempat1 = checkboxrm28($is_checkedKeempat1);
			//name Q perawat suntik shift pagi tanggal 1
			$perawatQPertama1 = $namaPerawatQPertama1[$i] ?? '-';
			$perawatQKedua1 = $namaPerawatQKedua1[$i] ?? '-';
			$perawatQKetiga1 = $namaPerawatQKetiga1[$i] ?? '-';
			$perawatQKeempat1 = $namaPerawatQKeempat1[$i] ?? '-';
			//name W perawat suntik shift sore tanggal 1
			$perawatWPertama1 = $namaPerawatWPertama1[$i] ?? '-';
			$perawatWKedua1 = $namaPerawatWKedua1[$i] ?? '-';
			$perawatWKetiga1 = $namaPerawatWKetiga1[$i] ?? '-';
			$perawatWKeempat1 = $namaPerawatWKeempat1[$i] ?? '-';
			//batas


			for ($baris = 2; $baris <= 16; $baris++) {
				// Ruang Pasien
				${"RuangPasien$baris"} = ${"RuangRawatan$baris"}[$i] ?? '-';

				// Jam Rawat
				foreach (['Pertama', 'Kedua', 'Ketiga', 'Keempat'] as $shift) {
					${"KjamRawat$shift$baris"} = ${"KetjamRawat$shift$baris"}[$i] ?? '-';

					// Checked
					$is_checked = ${$shift . "Check$baris"}[$i] ?? '0';
					${"check$shift$baris"} = checkboxrm28($is_checked);
				}

				// Perawat shift pagi (Q)
				foreach (['Pertama', 'Kedua', 'Ketiga', 'Keempat'] as $shift) {
					${"perawatQ$shift$baris"} = ${"namaPerawatQ$shift$baris"}[$i] ?? '-';
				}

				// Perawat shift sore (W)
				foreach (['Pertama', 'Kedua', 'Ketiga', 'Keempat'] as $shift) {
					${"perawatW$shift$baris"} = ${"namaPerawatW$shift$baris"}[$i] ?? '-';
				}
			}
			...
			// Ambil nilai checkbox per baris
			$is_checkedPertama = $jamcheck24jam[$i] ?? '0';
			$checkjampertama = checkboxrm28($is_checkedPertama);
			$is_checkedKedua = $jamcheck12jam[$i] ?? '0';
			$checkjamKedua = checkboxrm28($is_checkedKedua);
			$is_checkedKetiga = $jamcheck8jam[$i] ?? '0';
			$checkjamKetiga = checkboxrm28($is_checkedKetiga);
			$is_checkedKeempat = $jamcheck6jam[$i] ?? '0';
			$checkjamKeempat = checkboxrm28($is_checkedKeempat);
			$is_checkedKelima = $jamcheck3jam[$i] ?? '0';
			$checkjamKelima = checkboxrm28($is_checkedKelima);
			$is_checkedFarmasi = $farmasiCheck[$i] ?? '0';
			$checkfarmasi = checkboxrm28($is_checkedFarmasi);

			// === QR dokter ===
			$safe_dokter = $this->to_safe_filename($dokter_obat) . '.png';
			$path_qr_server_dokter = 'C:/xampp/htdocs/pmo/images/qrcodeIT/' . $safe_dokter;
			$path_qr_url_dokter = BASE_STORAGE . '/pmo/images/qrcodeIT/' . $safe_dokter;

			$ppa_with_qr = file_exists($path_qr_server_dokter)
				? '<img src="' . $path_qr_url_dokter . '" width="60" height="60" /><br/>'
				: '-<br/>' . $dokter_obat;
			$ppa_with_qr .= nl2br($ppa_val) . '<br/>' . $dokter_obat;



			$html1 .= '<tr>
					<td align="center">' . ($i + 1) . '</td>
					<td align="center">' . $tgljam . '</td>
					<td align="center">' . $nama_obat_val . '</td>
					<td align="center">' . $dosis . '</td>
					<td align="center">' . $checkjampertama . '</td>
					<td align="center">' . $checkjamKedua . '</td>
					<td align="center">' . $checkjamKetiga . '</td>
					<td align="center">' . $checkjamKeempat . '</td>
					<td align="center">' . $checkjamKelima . '</td>
					<td align="center">' . $rute . '</td>
					<td align="center">' . $ket . '</td>
					<td align="center">' . $durPertama . '</td>
					<td align="center">' . $durKedua . '</td>
					<td align="center">' . $ppa_with_qr . '</td>
					<td align="center">' . $checkfarmasi . '</td>
					<td align="center">Waktu</td>
					<td width="173">
						<table cellpadding="1">
							<tr>
								<td colspan="4">Ruangan : ' . $RuangPasien1 . '</td>
							</tr>
							<tr>
								<td>' . $KjamRawatPertama1 . '</td>
								<td>' . $KjamRawatKedua1 . '</td>
								<td>' . $KjamRawatKetiga1 . '</td>
								<td>' . $KjamRawatKeempat1 . '</td>
							</tr>
							<tr>
								<td>' . $checkPertama1 . '</td>
								<td>' . $checkKedua1 . '</td>
							...
							<tr>
								<td style="border-top:1px solid #000;" cellpadding="1">' . $perawatQPertama1 . '</td>
								<td style="border-top:1px solid #000;" cellpadding="1">' . $perawatQKedua1 . '</td>
								<td style="border-top:1px solid #000;" cellpadding="1">' . $perawatQKetiga1 . '</td>
								<td style="border-top:1px solid #000;" cellpadding="1">' . $perawatQKeempat1 . '</td>
							</tr>
							<tr>
								<td style="border-top:1px solid #000;" cellpadding="1">' . $perawatWPertama1 . '</td>
								<td style="border-top:1px solid #000;" cellpadding="1">' . $perawatWKedua1 . '</td>
								<td style="border-top:1px solid #000;" cellpadding="1">' . $perawatWKetiga1 . '</td>
								<td style="border-top:1px solid #000;" cellpadding="1">' . $perawatWKeempat1 . '</td>
							</tr>
						</table>
					</td>';
        ...
        <td width="173">
            <table cellpadding="1">
                <tr>
                    <td colspan="4">Ruangan : ' . ${"RuangPasien$baris"} . '</td>
                </tr>
                <tr>';
				foreach (['Pertama', 'Kedua', 'Ketiga', 'Keempat'] as $shift) {
					$html1 .= '<td>' . ${"KjamRawat$shift$baris"} . '</td>';
				}
				$html1 .= '</tr>
                <tr>';
				foreach (['Pertama', 'Kedua', 'Ketiga', 'Keempat'] as $shift) {
					$html1 .= '<td>' . ${"check$shift$baris"} . '</td>';
				}
				$html1 .= '</tr>
                <tr>';
				foreach (['Pertama', 'Kedua', 'Ketiga', 'Keempat'] as $shift) {
					$html1 .= '<td style="border-top:1px solid #000;" cellpadding="1">' . ${"perawatQ$shift$baris"} . '</td>';
				}
				$html1 .= '</tr>
                <tr>';
				foreach (['Pertama', 'Kedua', 'Ketiga', 'Keempat'] as $shift) {
					$html1 .= '<td style="border-top:1px solid #000;" cellpadding="1">' . ${"perawatW$shift$baris"} . '</td>';
				}
				$html1 .= '</tr>
            </table>
        </td>';
