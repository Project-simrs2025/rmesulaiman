...
<div class="content-wrapper container">
	<div class="page-content">
					...
					<nav aria-label="breadcrumb" class='breadcrumb-header float-start float-lg-end'>
						<ol class="breadcrumb">
							...
							<li class="breadcrumb-item active" aria-current="page"><a
									href="<?php echo site_url('backend/data_pasien/get_data_pasien/'.$id_pasien); ?>">Detail Kunjungan</a> </li>
		...
		<section id="input-validation">
			<h3><a href="<?php echo site_url('backend/data_pasien/get_data_pasien/'.$id_pasien); ?>"><i class="bi bi-chevron-left"></i><i
						...
						<div class="col-12 col-md-12">
							...
							<table class="table table-borderless float-start w-50">
								...
								<tr>
									...
									<td>:</td>
									<td><?php echo $nama_jenis_pasien; ?></td>
								</tr>
							</table>
							<table class="table table-borderless float-start w-50">
								<tr>
									<td class="w-25"><b>Tgl Admit</b></td>
									<td>:</td>
								...
								<tr>
									<td class="w-25"><b>Lama Rawat</b></td>
									<td>:</td>
									<td><?php echo $lama; ?></td>
								</tr>
							</table>
		...
		<script type="application/javascript">
			...
			$(document).ready(function () {
				...
				table2 = $('#mytablemenu').DataTable({
					...
					"order": [],
					"ajax": {
						"url": "<?php echo site_url('backend/admission/get_ajax_list_menu') ?>",
						"type": "POST",
						"data": function (data) {
			...
			function click_menu(nama_berkas) {
				...
				$.ajax({
					url: `<?= site_url('backend/admission/ajax_get_menu') ?>`,
					type: "GET",
					dataType: "JSON",
					data: {
						nama_berkas,
					...
					success: function (
						...
						if (selected_data) {
							canAdd = false
							const urlEdit = `<?= site_url('backend/data_pasien/edit'); ?>/${selected_data.id}`
							const urlView = `<?= site_url('backend/data_pasien/lihat'); ?>/${selected_data.id}`
							const deleteParam = {
								id: selected_data.id,
								nama_berkas
							}
							...
							listBerkas.push({
								...
								aksi: '<div class="d-flex flex-row gap-3">' +
									...
									`<button class="btn btn-outline-primary custom-icon"><a href="${urlView}" title="Lihat Berkas" class="text-primary custom-icon"><span class="bi bi-eye"></span></a></button>`,
							})
						}
			...
			// 	}
			// 	$.ajax({
			// 		url: `<?= site_url('backend/admission/delete') ?>`,
			// 		type: "POST",
			// 		dataType: "JSON",
			// 		data: {
			// 			id
			...
			$(document).ready(function () {
				$(document).on("click", "#add-form", function (e) {
					...
					window.location.href = site_url + "backend/data_pasien/tambah/" + link + "/" + id_kunjungan + "";
			...
			// 			$.ajax({
			// 				type: "post",
			// 				url: "<?php echo site_url('backend/admission/delete') ?>",
			// 				data: {
			// 					id_berkas: id_berkas,
			// 				},
			// 				dataType: "json",
