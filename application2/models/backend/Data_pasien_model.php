...
class Data_pasien_model extends CI_Model
{
	...
	{
		$this->postgres->select("admission.*, p.nama AS nama_pasien, lt.nama_lantai,sep.klsrawat as kelas, ru.nama_ruangan, k.nama AS nama_jenis_pasien,bed.no_bad,dokter.nama as nama_dokter,poli.name as nama_poli, p.nik, p.tanggal_lahir,p.no_bpjs as no_bpjs, pk.nama as nama_pekerjaan, jk.nama as jenkel, p.alamat, p.pendidikan_terakhir, agama.nama as nama_agama, p.no_handphone, dokter.path_ttd, suku.nama as nama_suku, penjamin.nama as nama_hub_pasien, penjamin.no_telepon as telp_hub_pasien, penjamin.alamat as alamat_hub_pasien,kunj.waktu_input as waktu_input, hub_keluarga.nama as hubungan_keluarga,to_char(kunj.waktu_input::timestamp, 'DD-MM-YYYY') as tgl_admit");
		$this->postgres->from('admission');
		$this->postgres->join('master_pasien p', 'p.id = admission.id_pasien', 'left');
