...
class Konsumen_model extends CI_Model
{
    ...
    {
        $this->postgres->select('k.id, k.nama, k.path_ttd, j.nama as nama_jabatan');
    ...
    public function update_path_ttd($id, $path)
    {
        $this->postgres->where('id', $id);
        return $this->postgres->update('master_karyawan', ['path_ttd' => $path]);
    }
}
